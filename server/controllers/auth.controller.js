      ...
      users => {
        ...
        bcrypt.hash(req.body.password, 10, (err, hashedCode) => {
          ...
          User.create(newUserEntry).then(
            data => {
              const token = jwt.sign({id: data.id}, Config(process.env.NODE_ENV).KEY, { expiresIn: 86400 * 365   });
    ...
    User.find({username: username}).then(
      data => {
        ...
        bcrypt.compare(password, data[0].password).then((matched) => {
          ...
          const token = jwt.sign({id: data[0].id}, Config(process.env.NODE_ENV).KEY, { expiresIn: 86400 * 365 });
